...
using streflop::Extended;
	...
	extern int __isnanl(StreflopDouble x);
...
// StreflopSimple and StreflopDouble are present in all configurations
	...
	inline StreflopSimple copysign(StreflopSimple x, StreflopSimple y) {return streflop_libm::__copysignf(x, y);}
#undef signbit
...
#undef isnormal
	...
	extern const StreflopSimple StreflopSimplePositiveInfinity;
	extern const StreflopSimple StreflopSimpleNegativeInfinity;
	...
	extern const StreflopSimple StreflopSimpleNaN;
...
// Additionally, using xxxf(number) avoids potential confusion
	...
	inline StreflopSimple copysignf(StreflopSimple x, StreflopSimple y) {return copysign(x, y);}
...
/*
	...
	inline StreflopDouble copysign(StreflopDouble x) {return streflop_libm::__copysign(x);}
	...
	inline int isnan(StreflopDouble x) {return streflop_libm::__isnanl(x);}
	...
	// Constants user may set
	extern const StreflopDouble StreflopDoublePositiveInfinity;
	extern const StreflopDouble StreflopDoubleNegativeInfinity;
	// Non-signaling NaN used for returning such results
	// Standard lets a large room for implementing different kinds of NaN
	// These NaN can be used for custom purposes
	// Threated as an integer, the bit pattern here may be incremented to get at least 2^20 different NaN custom numbers!
	// Note that when switching the left-most bit to 1, you can get another bunch of negative NaNs, whatever this mean.
	extern const StreflopDouble StreflopDoubleNaN;
